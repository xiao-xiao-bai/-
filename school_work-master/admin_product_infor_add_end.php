...
$product_count = $_POST['productcount'];
$product_money = $_POST['productmoney'];
...
if(is_uploaded_file($upfile["tmp_name"])){
    ...
    if(move_uploaded_file($upfile["tmp_name"],$path.$newfile)){
        ...
        //将图片的名称和路径存入数据库
        $new = $path . $newfile;
        $query = "INSERT INTO product(name,money,count,path,type)VALUES('{$product_name}','{$product_money}','{$product_count}','{$new}','{$product_type}')";
        $result = $pdo -> query($query);
        if($result){
            echo "<script>alert('商品添加成功');location='admin_product_infor_show.php';</script>";
        }
        else{
            echo "<script>alert('商品添加失败');location='admin_product_infor_show.php';</script>";
        }
    }else{
        die("上传文件失败！");
        echo "<script>alert('商品添加失败');location='admin_product_infor_show.php';</script>";
